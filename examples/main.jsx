...
class SketchFieldDemo extends React.Component {
  ...
  _onSelection = (target) => {
    ...
    if (target && target.type === "i-text") {
      this.setState({ fontSize: target.fontSize || 16 });
    }
  };

  _onScalling = (target) => {
    if (target.type === "i-text" && this._fontSizeInput) {
      this._fontSizeInput.value = (target.fontSize * target.scaleY).toFixed(0);
    }
  };

  onModified = (target) => {
    if (target.type === "i-text") {
      target.fontSize = (target.fontSize *= target.scaleY).toFixed(0);
      target.scaleY = 1;
      target.scaleX = 1;
      target._clearCache();
                ...
                <CardContent>
                  ...
                  <div className="row">
                    ...
                    {this.state.selection &&
                      this.state.selection.type === "i-text" && (
                        <div className="col-lg-3">
                          <TextField
                            inputRef={(c) => (this._fontSizeInput = c)}
                            label="Размер"
                            type="number"
